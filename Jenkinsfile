                ...
                try {
                  stage( "starting jetty app ${jettyVersion}" ) {
                    withEnv( ["JAVA_HOME=${tool "$jdk"}"] ) {
                      ...
                      {
                        jettyStart =
                                "${env.JAVA_HOME}/bin/java $serverVmOptions -jar ../jetty-distribution-$jettyVersion/start.jar -Djetty.version=${jettyVersion}"
                      }
                      // TODO make this configuration easier
                ...
                stage( 'run probe' ) {
                  ...
                  timeout( time: 6, unit: 'HOURS' ) {
                    withEnv( ["JAVA_HOME=${tool "$jdkLoad"}"] ) {
                      ...
                      "--loader-rate $loaderRate --loader-number $loaderInstancesNumber --jetty-version ${jettyVersion}"
                    }
                  }
